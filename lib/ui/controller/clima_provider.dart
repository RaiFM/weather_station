...
class ClimaProvider extends ChangeNotifier {
  ...
  ClimaProvider({required this.listarLocaisPorLatLonUc, required this.listarLocaisPorNomeUc, required this.listarLocaisSalvosUc});
 ...
 List<ClimaModel?> _allClimaSalvos = [];
  ...
  Future<List<ClimaModel?>> getClimaLatLon(double lat, double lon) async {
    _allClimaPrincipal = [];
   return _allClimaPrincipal = await listarLocaisPorLatLonUc.getInstance.buscarPorLatLon(lat, lon);
  }

  Future<List<ClimaModel?>> getClimaNome(String nome) async {
    _allClimaPrincipal = [];
   return _allClimaPrincipal = await listarLocaisPorNomeUc.getInstance.buscarPorNome(nome);
  }
